...
root_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'hand_gesture_dataset')
...
class TemporalAttention(nn.Module):
    def __init__(self, channel_dim):
        super().__init__()
        self.attention_fc = nn.Linear(channel_dim, 1)

    def forward(self, x):
        # x: (B, C, T)
        x = x.permute(0, 2, 1)     # (B, T, C)
        attn_scores = self.attention_fc(x).squeeze(-1)   # (B, T)
        attn_weights = torch.softmax(attn_scores, dim=1) # (B, T)
        attn_weights = attn_weights.unsqueeze(-1)        # (B, T, 1)
        weighted = (x * attn_weights).sum(dim=1)         # (B, C)
        return weighted
...
class ResNet3D(nn.Module):
    def __init__(self, num_classes, dropout_prob=0.5):
        ...
        # Loading the pre-trained R(2+1)D model
        self.backbone = r2plus1d_18(pretrained=True)
        
        # Get the feature dimension before FC layer
        channel_dim = self.backbone.fc.in_features
        
        # Remove the FC layer from backbone
        self.backbone.fc = nn.Identity()
        
        # Add temporal attention module
        self.temporal_attention = TemporalAttention(channel_dim=channel_dim)
        
        # Add new FC layer with dropout
        self.fc = nn.Sequential(
            nn.Dropout(dropout_prob),
            nn.Linear(channel_dim, num_classes)
        )

    def forward(self, x):
        # Extract features from backbone (before global pooling)
        # Manually pass through conv layers to get features with temporal dimension
        feat = x
        for name, module in self.backbone.named_children():
            if name not in ['avgpool', 'fc']:
                feat = module(feat)
        
        # Reduce spatial dimensions (H, W) → (B, C, T)
        feat = feat.mean(dim=[3, 4])
        
        # Apply temporal attention → (B, C)
        attn_output = self.temporal_attention(feat)
